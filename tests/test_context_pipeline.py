...
验证: ctx 贯穿 handler，LLM 收到正确用户身份
覆盖: Case 007, 008
"""
...
def _inject_user_prompt(user, event=None):
    """Inline replacement for request_context.inject_user_prompt."""
    from core.hierarchy_resolver import inject_org_context

    parts = [f"你正在与 {user['name']}（{user['role']}，{user['team']}）对话。你是他的工班助手。"]
    if event:
...
def test_context_built():
    """Case 008: 当前用户识别"""
    assert CURRENT_USER["name"] == "李林骁"
    assert CURRENT_USER["role"] == "工班长"
    assert CURRENT_USER["team"] == "铁炉西工班"
    print("  ✓ user identity: 李林骁 role=工班长")

    prompt = _inject_user_prompt(CURRENT_USER)
    assert "李林骁" in prompt, f"prompt missing 李林骁: {prompt}"
    assert "工班长" in prompt, f"prompt missing 工班长: {prompt}"
    print("  ✓ prompt contains 李林骁+工班长")


def test_context_in_handler():
    """Case 007: Event进入Handler — ctx 传递验证"""
    event = {"requester": "王亮", "executor": "李林骁", "deadline": "14日"}
    prompt = _inject_user_prompt(CURRENT_USER, event)
...
def test_context_flow():
    """验证 ctx 从 handle() 经过 LLM prompt 前包含正确 org 上下文"""
    from core.hierarchy_resolver import inject_org_context

    event = {"requester": "王亮", "executor": "李林骁", "target": "各班组"}
    org = inject_org_context(event, CURRENT_USER)
...
def test_llm_forbid_hello():
    """Case 008 forbid: prompt 应指向当前用户而非 requester"""
    event = {"requester": "王亮", "executor": "李林骁", "target": "各班组"}
    prompt = _inject_user_prompt(CURRENT_USER, event)
