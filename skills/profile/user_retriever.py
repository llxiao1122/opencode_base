...
ORG_PATH = ROOT / "state" / "org_memory.json"
...
def get_profile_for_reasoning(name: str) -> dict:
    """Lean profile structure optimized for LLM reasoning analysis.

    Removes internal IDs, pre-computes summaries.
    """
    ctx = get_person_context(name)
    if "error" in ctx:
        return ctx
    ...
    return {
        "name": name,
        "role": ctx["role"]["current"],
        "activity": {
            "events": ctx["activity"]["events_total"],
            "tasks": ctx["activity"]["tasks_total"],
            "as_executor": ctx["activity"]["executor_count"],
        },
        "execution": {
            "completed": ctx["execution"]["completed"],
            "pending": ctx["execution"]["pending"],
            "completion_rate": ctx["execution"]["completion_rate"],
            "avg_response_hours": ctx["execution"]["avg_response_hours"],
            "main_task_types": list(ctx["execution"]["task_types"].keys())[:5],
        },
        "common_tasks": ctx["responsibility"]["common_tasks"],
        "patterns": [{"type": p["type"], "fact": p["fact"]} for p in ctx["patterns"]],
        "top_collaborators": [r["person"] for r in ctx["relations"][:5]],
        "trends": {
            "monthly_summary": ctx["trends"].get("monthly", []),
            "completion_cycle_days": (
                ctx["trends"].get("completion_cycle", {}).get("avg_days")
            ),
        },
    }
