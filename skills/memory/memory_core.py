...
class MemoryCore:
    def __init__(self, root_path=""):
        ...
        import re as _re
        _key = os.environ.get("LLM_API_KEY", "") or os.environ.get("DEEPSEEK_API_KEY", "")
        if not _key:
            for _p in [Path.home() / ".config" / "opencode" / "opencode.jsonc",
                       Path(__file__).resolve().parent.parent.parent / ".opencode" / "opencode.jsonc"]:
                if _p.exists():
                    try:
                        _raw = _re.sub(r'(?<!\:)//.*$', '', _p.read_text(), flags=_re.MULTILINE)
                        _raw = _re.sub(r'/\*.*?\*/', '', _raw, flags=_re.DOTALL)
                        _raw = _re.sub(r',\s*([}\]])', r'\1', _raw)
                        _rk = json.loads(_raw).get("provider", {}).get("deepseek", {}).get("options", {}).get("apiKey", "")
                        if isinstance(_rk, str):
                            if _rk.startswith("{env:") and _rk.endswith("}"):
